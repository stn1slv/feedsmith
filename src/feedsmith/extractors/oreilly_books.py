...
# non-book, undated) does not starve the feed; the service layer truncates.
_LIMIT_BUFFER = 10
...
class OreillyBooksExtractor:
    ...
    def fetch(self, cfg: FeedConfig, client: httpx.Client) -> list[Post]:
        ...
        posts = [post for item in results if isinstance(item, dict) if (post := self._to_post(item)) is not None]
    ...
    def _to_post(self, item: dict[str, Any]) -> Post | None:
        ...
        or lack a usable id, title, link, or catalog-add date — all expected in a
        broad search response and out of place in this feed.
        """
        fmt = item.get("format")
        ...
        published = self._parse_datetime(item.get("date_added"))
        web_url = item.get("web_url")
        title = _clean(item.get("title"))
        book_id = item.get("ourn") or item.get("archive_id") or item.get("isbn")
        if published is None or not isinstance(web_url, str) or not web_url or not title or not book_id:
            return None
